...
export default ({ items, item, collection, add }) => {
  ...
  return items.length === 0
    ...
    : h(`div.master-detail.d-flex.flex-row.flex-1`, [
        ...
        item
          ? h(`div.detail.pl-sm-4.d-flex.flex-column.flex-1${h3.state.selection ? '' : '.hide-sm'}`, [
              ...
              h('div.d-flex.flex-column.flex-1.scrollable-area', [
                ...
                collection === 'snippets' &&
                  h('pre', h(`code#item-content.language-${data.language.value}`, { $onrender: (node) => Prism.highlightElement(node) }, data.code.value)),
              ]),
            ])
